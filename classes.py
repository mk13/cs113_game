...
class Player(Rect2):
    ...
    def __call__(self, input, arena_map):
        ...
        if not self.conditions[STUN] and not self.conditions[SILENCE]:
            self._handle_inputs(input,arena_map)
    ...
    def _handle_inputs(self, input, arena):
        i,button = self._priority_inputs(input)
        if input.DROP_SKILL and i != -1:  # Drop skill pressed
            self.__dict__[button] = 0

        elif not input.DROP_SKILL:  # Drop skill not pressed
